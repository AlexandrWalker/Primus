...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const titleLines = document.querySelectorAll('[data-splitting="line"]');
  ...
  function mainScriptAfterPreloader() {
    ...
    const fadeUpItems = document.querySelectorAll('[data-transform="fadeUp"]');
    fadeUpItems.forEach(fadeUpItem => {
      const tl = gsap.timeline({
        paused: true
      });
      tl.fromTo(fadeUpItem,
        {
          y: '+5%',
          opacity: 0,
          rotate: '20deg',
    ...
    if (footer) {
      gsap.from(footer, {
        ...
        onStart: function () {
          footer.classList.add('footerAnimStart');
        },
      });
    }
  }
